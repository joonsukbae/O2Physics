...
struct JetCrossSectionEfficiency {
  ...
  void init(InitContext&)
  {
    ...
    if (doprocessCrossSectionEfficiencyCollRecoFirst) {
  ...
  void processCrossSectionEfficiencyCollRecoFirst(aod::JetMcCollisions::iterator const& mccollision,
                                                  ...
                                                  aod::JBCs const&)
  {
    ...
    for (int s = kBinCRF_Inel; s <= kBinCRF_N; ++s) {
      if (!pass[s])
        break;
      registry.fill(HIST("h_mccollisions_eventselection_collRecoFirst"), static_cast<double>(s));
      sMax = s;
    }
    if (sMax == 0) {
      return;
    ...
    for (auto const& jet : jets) {
      ...
      for (int s = kBinCRF_Inel; s <= sMax; ++s) {
        registry.fill(HIST("h2_jet_pt_part_eventselection_collRecoFirst"), jet.pt(), static_cast<double>(s));
      }
    }
  }
  PROCESS_SWITCH(JetCrossSectionEfficiency, processCrossSectionEfficiencyCollRecoFirst,
                                                ...
                                                aod::JetMcCollisions const& allMcCollisions)
  {
    ...
    for (int s = kBinBBF_Inel; s <= kBinBBF_N; ++s) {
      if (!pass[s])
        break;
      registry.fill(HIST("h_mccollisions_eventselection_bcBitsFirst"), static_cast<double>(s));
      sMax = s;
    }
    if (sMax == 0) {
      return;
    ...
    for (auto const& jet : jets) {
      ...
      for (int s = kBinBBF_Inel; s <= sMax; ++s) {
        registry.fill(HIST("h2_jet_pt_part_eventselection_bcBitsFirst"), jet.pt(), static_cast<double>(s));
      }
    }
  }
